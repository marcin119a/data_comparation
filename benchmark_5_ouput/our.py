...
from model_selection_new import backward_elimination
...
def main_block():
    ...
    for i in range(tumor.shape[1]):
        first_col = tumor[:, i]
        try:
            best_columns, b, estimation_exposures = backward_elimination(first_col, signaturesCOSMIC, threshold=0.01, mutation_count=None, R=100, significance_level=0.01)
            print(best_columns)
        except:
            continue
        r = save_to_dataframe(best_columns, estimation_exposures[0], f'{i}', 'Test')
